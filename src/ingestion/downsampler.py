...
def downsample_image(src: Path, dst: Path, *, dpi: int = TARGET_DPI) -> Path | None:
    ...
    Returns the destination path, or None if the image could not be processed.
    """
    ...
    try:
        ...
        logger.warning(f"Skipping unreadable image: {src.name} ({exc})")
        return None
