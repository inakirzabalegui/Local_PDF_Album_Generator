...
def api_pick_folder():
    ...
    try:
        ...
        folder_path = filedialog.askdirectory(
            title="Selecciona carpeta con fotos",
...
def api_bootstrap():
    ...
    if len(source_path_str) >= 2 and source_path_str[0] in ('"', "'") and source_path_str[-1] == source_path_str[0]:
    ...
    try:
        ...
        if not source_path.is_dir():
            return jsonify({'success': False, 'error': f"'{source_path}' no es un directorio"}), 400
        
        # Reject if user accidentally picked an _album workspace folder
        if source_path.name.endswith('_album') or (source_path / 'global_config.yaml').exists():
            return jsonify({
                'success': False,
        ...
        if not has_image:
            return jsonify({'success': False, 'error': 'No se encontraron imágenes en la carpeta'}), 400
        ...
        if not workspace.exists():
            ...
            _bootstrap_workspace(source_path, workspace)
        else:
...
def _bootstrap_workspace(source_path: Path, workspace: Path) -> None:
    ...
    if scan_result.cover_photos:
        logger_instance.info(f"✓ Carpeta 'portada' encontrada: {len(scan_result.cover_photos)} foto(s)")
    else:
        logger_instance.info("○ Carpeta 'portada' no encontrada")
    
    if scan_result.backcover_photos:
        logger_instance.info(f"✓ Carpeta 'contraportada' encontrada: {len(scan_result.backcover_photos)} foto(s)")
    else:
